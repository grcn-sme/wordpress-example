...
/** @param {string} path - should begin with '#/xxx' */
async function navigatePage(pathname) {
  // fetch resources
  // #/pathname/xxx/yyy
  ...
  // load page
  const src = `./page${pathname === '/' ? '' : pathname}/_.html`;
  ...
  const lastResource0 = _loadSharedResources(pathname, modules, appBody);
  const lastResource1 = _loadIndividualResources(pathname, modules, appBody);
  const lastResource2 = _loadSharedResources2(pathname, modules, appBody);
...
// init page
navigatePage(location.pathname);
processSearchParams(new URLSearchParams(location.search));
